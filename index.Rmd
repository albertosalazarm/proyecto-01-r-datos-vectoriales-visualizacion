...
```{r}
asia_paises_vida <- world[world$continent == "Asia" & world$lifeExp > 80 , ]
asia_paises_vida <- na.exclude(asia_paises_vida)

ggplot(asia_paises_vida, aes(x=reorder(name_long, -lifeExp), y=lifeExp,  )) +
  ggtitle("Países de Asia con esperanza de vida mayor a 80 años") +
  xlab("País") + 
  ylab("Esperanza de vida") +  
    geom_col() +
  geom_text(aes(label=lifeExp), vjust=-0.3, size=3.5 ) +
  coord_cartesian(ylim = c(80, 84)) 
...
plot(oceania_paises["lifeExp"], main="Esperanza de vida en Oceania")
...
ca_paises_vida<- centro_america[centro_america$lifeExp >= 74, ]
plot(ca_paises_vida["lifeExp"], main = 'Países de CA con esperanza de vida mayor o igual a 74 años')
...
```{r}
amsur <- world[ world$continent == "South America" ,]
amsur_condi <- amsur[amsur$area_km2 >= 2500000 | amsur$pop >= 40000000 ,]

#Para la Variable área
plot(amsur_condi["area_km2"], main="Países de Amer. del Sur con área superior a 2.5 millones de km2")

#Para la Variable población
plot(amsur_condi["pop"], main="Países de Amer. del Sur con población superior a 40 millones")
